...
export default function CustomerOrders({ initialOrders, currentUser }) {
    ...
    // Filter orders locally by ID, address, or phone to help customers search their order history
    ...
    // =========================================================================
    // 🗑️ ORDER ACTION: CANCEL ORDER (DELETE /api/orders/:orderId) [4]
    // =========================================================================
    const handleCancelOrder = async () => {
        ...
        try {
            // Send DELETE cancellation signal to backend microservice route [4]
            await axios.delete(`/api/orders/${orderId}`);

            // Optimistically update status to 'Cancelled' on success [4]
            const nextOrdersList = orders.map(o => 
                (o.id === orderId || o._id === orderId) ? { ...o, status: 'Cancelled' } : o
            );
            setOrders(nextOrdersList);
            setActiveOrder({ ...activeOrder, status: 'Cancelled' });
        } catch (err) {
            console.error("Failed to cancel order:", err);
            const errMsg = err.response?.data?.errors?.[0]?.message || "Failed to cancel your order.";
    ...
    return (
        <div className="bg-orange-50/50 text-zinc-800 min-h-[calc(100vh-73px)] flex flex-col font-sans selection:bg-lime-200">
            {/* Nav Header is loaded dynamically in _app.js layout [2] */}
                    ...
                    <div className="flex-grow overflow-y-auto pr-2 space-y-4 pb-8 custom-scrollbar">
                        {filteredOrders.length === 0 ? (
                            <div className="bg-white rounded-3xl p-10 text-center border border-zinc-100 shadow-sm">
                                <span className="material-symbols-outlined text-zinc-300 text-5xl mb-2">shopping_bag</span>
                                <p className="text-zinc-500 font-medium text-sm">You haven't placed any orders yet.</p>
                            </div>
                        ) : (
                            filteredOrders.map((order) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            {isCompleted && (
                                                <span className="bg-lime-50 text-lime-700 border border-lime-200 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
                                                    <span className="w-1.5 h-1.5 rounded-full bg-lime-500"></span> Delivered
                                                </span>
                                            )}
                                            {isCancelled && (
                                        ...
                                        <h3 className="font-bold text-zinc-800 text-lg mb-1 truncate">{order.product?.title || "Item Order"}</h3>
                    ...
                    {activeOrder ? (
                        <div className="flex flex-col h-full animate-enter">
                            ...
                            <div className="px-6 py-4 border-b border-zinc-100 flex justify-between items-center shrink-0">
                                <div>
                                    <h2 className="text-xl font-extrabold text-zinc-800 font-headline">Order Details</h2>
                                    <p className="text-xs text-zinc-400 mt-0.5 font-body">Placed on {formatDate(activeOrder.createdAt)}</p>
                                </div>
                            </div>

                            <div className="flex-grow overflow-y-auto p-6 space-y-6 custom-scrollbar">
                                
                                {/* Delivery Address Info Block */}
                                <div className="bg-zinc-50 rounded-2xl p-5 border border-zinc-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-zinc-800 text-base">Your Shipping Details</h4>
                                ...
                                <div className="flex justify-end gap-3">
                                    ...
                                    {activeOrder.status !== 'complete' && activeOrder.status !== 'Complete' && activeOrder.status !== 'Cancelled' && activeOrder.status !== 'cancelled' ? (
                                        <button 
                                            ...
                                            className="px-6 py-3 rounded-xl border border-red-600 text-red-600 font-bold hover:bg-red-50 transition-colors focus:outline-none text-sm"
                                        >
...
// Next.js SSR Context Fetcher [2]
CustomerOrders.getInitialProps = async (context, client, currentUser) => {
    ...
    try {
        // Query the Orders service index endpoint (which returns customer-specific filtered orders) [2]
        ...
        console.error("Failed to fetch customer orders catalog history:", err.message);
        return { 
            initialOrders: [], 
            currentUser 
        };
