...
import { OrderCancelledEvent, OrderStatus } from "@d-ziet/common-lib";
import { io } from "../../../app";
...
it('archives a conversation when an order is cancelled', async () => {
    const { listener, data, msg } = await setup();
    await listener.onMessage(data, msg);
...
it('emits a socket event to the vendor and customer when an order is cancelled', async () => {
